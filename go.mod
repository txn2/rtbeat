module github.com/txn2/rtbeat

go 1.26.0

// Elastic libbeat requires these replace directives from its own go.mod;
// Go modules do not apply a dependency's replaces transitively, so consumers
// must mirror them. Sourced from github.com/elastic/beats/v7@v7.17.29.
// Mirror the full replace block verbatim from github.com/elastic/beats/v7@v7.17.29
// go.mod. We only import a few libbeat packages today, but keeping the entire
// upstream set means a future dependency bump that drags in more of beats won't
// break against a fork-expecting package (e.g. docker/docker => docker/engine).
replace (
